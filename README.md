...
- **复习统计**：按科目/分类查看掌握情况，回顾近 30 天的复习趋势和遗忘次数。
- **数据备份**：一键导出包含全部错题、图片和复习记录的 JSON 文件，也可从备份恢复。
...
src/app/api/          上传、临时附件清理、OCR、错题、复习和备份接口
src/components/       工作台、OCR 上传、共享表单、详情、编辑和复习组件
src/lib/              SQLite、图片存储、OCR、SM-2、输入校验和共享类型
public/               静态资源
...
应用首次运行时会在 `data/errors.db` 创建 SQLite 数据库。新上传的图片先写入 `data/tmp-images/`，保存错题后才移动到 `data/images/`；取消或过期的临时附件会被清理。图片附件以子表 `record_images` 保存，复习历史记录在 `review_log` 中，用于统计页的趋势展示。

应用内的「备份」页面（`/backup`）可以把全部错题、图片和复习记录导出为一个 JSON 文件，也可以从备份文件恢复；导入会覆盖当前数据，操作前请先导出。
